...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    h1 = np.dot(X,W1) + b1                 # fully connected
    l = np.maximum(h1,np.zeros(h1.shape))  #hidden layer relu
    scores = np.dot(l,W2) + b2            # fully connected
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    # division by sum of each row, reshaped to a column
    sum_rows = np.sum(np.exp(scores), axis = 1).reshape(-1,1)
    # print sum_rows
    # print scores[np.arange(N),y]
    normalize_scores = np.exp(scores) / sum_rows
    data_loss = (np.sum(-np.log(normalize_scores[np.arange(N),y]))) / N
    ...
    ind = np.zeros(dscores.shape)
    ind[range(N),y] = 1
    dscores = (dscores - ind) / N
    grads['W2'] = np.dot(l.T,dscores)
  ...
  def predict(self, X):
    ...
    l = np.maximum(h1,np.zeros(h1.shape))
    s = np.dot(l,self.params['W2']) + self.params['b2']
